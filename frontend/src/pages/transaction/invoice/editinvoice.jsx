...
const EditInvoice = () => {
	let breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'Invoice', to: '/transaction/invoice' }, { title: 'Edit' }];
	...
	useEffect(() => {
		...
		axiosServices.get(`/api/invoice/getinvoice/${id}`)
			.then(res => {
				if (res.data.invoice)
			...
			.catch(err => navigate('/transaction/invoice'))
	}, [])
	...
	const handleSubmit = () => {
		axiosServices.post(`/api/invoice/modify/${id}`, { invoice })
			.then(res => {
				if (res.data.success)
					openSnackbar({
						open: true,
