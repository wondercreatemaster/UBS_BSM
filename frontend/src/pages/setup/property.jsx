...
import PropertyView from 'sections/setup/property/PropertyView';
import
{
  DebouncedInput,
  HeaderSort,
...
function ReactTable({ data, columns, rowCount, setPage, setPropertiesPerpage, setSearchword, setPhaseFiltering, setTypeFiltering, phaseFiltering, typeFiltering })
{
  ...
  {
    navigate('/setup/property/add');
  };

  const handleGenerateProperty = () =>
  {
    navigate('/setup/property/generate')
  }
    ...
    <MainCard content={false}>
      <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ p: 3 }}>
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <Button variant="contained" startIcon={<Add />} onClick={handleAddProperty} size="large">
            Add Property
          </Button>
          <Button variant="contained" onClick={handleGenerateProperty} size="large">
            Generate
          </Button>
        </Stack>
      </Stack>
                    ...
                    {row.getIsExpanded() && (
                      <TableRow sx={{ '&:hover': { bgcolor: `${backColor} !important` } }}>
                        <TableCell colSpan={row.getVisibleCells().length}>
                          <PropertyView data={row.original} />
                        </TableCell>
                      </TableRow>
                    )}
                  </Fragment>
...
export default function Property()
{
            ...
            <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
              ...
              <Tooltip title="Edit">
                <IconButton
                  ...
                  onClick={(e) =>
                  {
                    navigate(`/setup/property/edit/${row.original._id}`)
                    e.stopPropagation();
                  }}
                >
                  <Edit />
  ...
  let breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'Property' }];

  return (
    <>
      <Breadcrumbs custom heading="Property" links={breadcrumbLinks} />
