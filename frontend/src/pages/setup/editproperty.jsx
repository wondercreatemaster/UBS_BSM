...
import { Box, Button, CircularProgress, Grid, MenuItem, OutlinedInput, Select, TextField, Typography } from "@mui/material"
...
const EditProperty = () =>
{
  ...
  {
    setIsloading(true)
    axiosServices.get(`/api/property/${id}`)
      .then(res =>
      {
        if (res.status == 200)
        {
      ...
      .catch(err => navigate('/setup/property'))
  }, [])
  ...
  let breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'Property', to: '/setup/property' }, { title: 'Edit' }];

  return <div>
    <Breadcrumbs custom heading="Edit Property" links={breadcrumbLinks} />
    <MainCard>
      {
        isLoading ?
          <div>
          ...
          <>
            <Grid container spacing={5} alignItems="baseline">
              <Grid item md={12}>
                <Typography className="text-2xl">Unit No: {property.UNITNO}</Typography>
              </Grid>
