...
const EditOwner = () => {
  ...
  let breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'Ownership', to: '/setup/ownership' }, { title: 'Edit' }];
  ...
  useEffect(() => {
    const fetchowner = async () => {
      axiosServices.get(`/api/owner/getownerbyid/${id}`)
        .then(res => {
  ...
  const handleSubmit = () => {
    axiosServices.post('/api/owner/modify', { owner })
      .then(res => {
        if (res.data.success)
          openSnackbar({
            open: true,
  ...
  return (
    <>
      <Breadcrumbs custom heading="Edit Owner" links={breadcrumbLinks} />
      <MainCard>
        {
          owner &&
          <>
            <Typography className='text-3xl'>Unit No {owner.UNITNO}</Typography>
