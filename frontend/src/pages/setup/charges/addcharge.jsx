...
const AddCharge = () => {
  ...
  const handleSubmit = () => {
    axiosServices.post('/api/charge/add', { charge })
      .then(res => {
        if (res.data.success)
          openSnackbar({
            open: true,
  ...
  let breadcrumbLinks = [{ title: 'Home', to: APP_DEFAULT_PATH }, { title: 'Charges', to: "/setup/charges" }, { title: 'Add' }];

  return <>
    <Breadcrumbs custom heading="Add Charge" links={breadcrumbLinks} />
    <MainCard>
            ...
            <Box className="p-[20px]">
              <Grid container spacing={2} direction='column'>
                <Grid item className="grid grid-cols-6 items-center">
                  ...
                  <TextField value={charge.CHARCODE} onChange={e => updateCharge("CHARCODE", e.target.value)} />
                </Grid>
                <Grid item className="grid grid-cols-6 items-center">
